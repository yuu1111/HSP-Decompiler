...
namespace KttK.HspDecompiler.Ax2ToAs.Data
{
    ...
    internal class Token
    {
        ...
        private static Token GetToken(int offset)
        {
            ...
            Token ret = new Token();
            ret.id = offset / 2;
            ret.fValue = data.TokenData[offset];
            offset++;
            ret.fType = data.TokenData[offset];
            offset++;
            ret.size = 2;
            if ((ret.fType & 0x01) != 0)
            {
            ...
            {
                ret.size += 2;
                ret.IfJumpTo = BitConverter.ToInt16(data.TokenData, offset);
                offset += 2;
            ...
            if ((ret.fType & 0x80) != 0) // &&(ret.fValue==0) )
            {
                ret.fType ^= 0x80;
                ret.size += 4;
                ret.fValue = BitConverter.ToInt32(data.TokenData, offset);
                offset += 4;
            }
