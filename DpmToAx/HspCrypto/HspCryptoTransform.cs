...
namespace KttK.HspDecompiler.DpmToAx.HspCrypto
{
    ...
    {
        private XorAddTransform xorAdd;

        internal XorAddTransform XorAdd
        {
            get { return this.xorAdd; }
            set { this.xorAdd = value; }
        }

        public override string ToString()
        {
            return this.xorAdd.ToString();
        }
        ...
        internal byte[] Encryption(byte[] plain)
        {
            ...
            {
                encrypted[i] = this.xorAdd.Encode(XorAddTransform.Dif(plain[i], prevByte));
                prevByte = plain[i];
            }
        ...
        internal byte[] Decryption(byte[] encrypted)
        {
            ...
            for (int i = 0; i < encrypted.Length; i++)
            {
                byte plainByte = this.xorAdd.Decode(encrypted[i]);
